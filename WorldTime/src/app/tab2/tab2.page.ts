...
export class Tab2Page {
  ...
  parse() {
    this.Data.forEach(element => {
      const Misto:Misto = {
        misto: element.split("/")[0],
        Mesto: element.split("/")[1]
      }
      this.Mista.push(Misto);
    });    
  }
  Add(index:number)
  {
    this.timeService.addData("http://worldtimeapi.org/api/timezone/"+this.Mista[index].misto +"/"+this.Mista[index].Mesto);
  }
